...
/// \param event custom event to track
///
...
/// This API is subject to SDK guardrails — there is a <em>60-second throttling period</em> between consecutive calls. Use it only when timely delivery is important.
/// \param event custom event to track at high priority
///
+ (void)sendImmediateWithEvent:(id <SFMCSdkEvent> _Nonnull)event;
/// Schedules a flush of any queued events. The flush is subject to SDK guardrails — notably, a <em>60-second throttling period</em> between consecutive flush calls. If a flush was recently performed, the request will be deferred until the throttle window has elapsed.
